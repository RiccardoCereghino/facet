...
func dependencyEdge(repo string, number int, direction string) ([]IssueRef, error) {
	// Conditional, like every other REST read here: a dependency list that has
	// not changed answers 304 and costs nothing. Where a second page exists the
	// conditional form cannot be used -- one cache key cannot hold a partial
	// list -- so it falls back to the full paginated read.
	path := fmt.Sprintf("repos/%s/issues/%d/dependencies/%s?per_page=%d", repo, number, direction, restPerPage)
	var out []byte
