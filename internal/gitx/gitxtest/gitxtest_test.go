...
func TestFailMatchesReturnErr(t *testing.T) {
	...
	if _, err := r.Run("", nil, "fetch", "origin"); !errors.Is(err, want) {
		t.Fatalf("Run(fetch) = %v, want %v", err, want)
	}
	if _, err := r.RunTimeout("", nil, 0, "fetch"); !errors.Is(err, want) {
		t.Fatalf("RunTimeout(fetch) = %v, want %v", err, want)
	}
}

func TestRealIsDelegatedToWhenFailDoesNotMatch(t *testing.T) {
	delegate := &Runner{} // stands in for a real Runner in this test
	r := &Runner{
		Real: delegate,
		Fail: func(args []string) bool { return len(args) > 0 && args[0] == "fetch" },
		Err:  errors.New("boom"),
	...
	if len(delegate.Calls) != 1 || delegate.Calls[0][0] != "status" {
		t.Fatalf("Real.Calls = %v; want the delegated call recorded on Real too", delegate.Calls)
	}
}
...
func TestOnCallRunsBeforeFailIsChecked(t *testing.T) {
	...
	r := &Runner{
		...
		Fail:   func(_ []string) bool { return true },
		Err:    errors.New("boom"),
	}
	_, _ = r.Run("", nil, "clone")
